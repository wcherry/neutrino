 ...
 *
 * The property that matters is that the correction is *directional*: sharpening along the axis a
 * photo smeared along must recover more edge contrast than sharpening across it. An ordinary
 * isotropic sharpen would score the same either way, and would be no use for camera shake.
 */
...
import {
  ...
  scaleKernel,
  type PixelBuffer,
} from '../../app/(apps)/photos/editor/deblur';
