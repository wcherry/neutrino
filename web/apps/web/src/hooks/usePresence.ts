...
//
// The backend (src/docs/collab/api.rs) implements the y-websocket binary protocol:
//
//  Type 0 — sync:
//    [varint 0] [varint sub] [varint payload_len] [payload_bytes]
//      sub=0  SyncStep1  payload = client's state vector
//      sub=1  SyncStep2  payload = Yjs update (all docs missing from sv)
//      sub=2  Update     payload = incremental Yjs update
//
//  Type 1 — awareness (our JSON presence messages):
...
// Text colors from Avatar.module.css color-0 … color-7 (same order, same hash)
